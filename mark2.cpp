...
vector<vector<int>> processingTimes = loadProcessingTimesFromJSON("your_file_path.json");

// Function to calculate makespan for a given permutation of jobs
int calculateMakespan(const vector<int>& permutation) {
...
void tabuSearchWorker(int threadID, int maxIterations, int tabuListSize, vector<int>& localBestSolution, int& localBestMakespan) {
...
int main() {
    ...
    int numThreads = 4;
    int maxIterationsPerThread = 1000;
    int tabuListSize = 25;
    ...
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back(tabuSearchWorker, i, maxIterationsPerThread, tabuListSize, ref(localBestSolutions[i]), ref(localBestMakespans[i]));
    }
