...
// Funkcja oceniająca funkcję celu dla danego sąsiada.
int evaluateNeighbor(const Solution& neighbor, const vector<vector<int>>& processingTimes) {
    ...
    return neighborObjective;
}

// Wątek odpowiedzialny za generowanie sąsiadów i ocenę ich funkcji celu.
void generateAndEvaluateNeighbors(const Solution& currentSolution, const vector<vector<int>>& processingTimes,
    ...
    for (int iter = 0; iter < maxIterations; ++iter) {
        ...
        {
            lock_guard<mutex> lock(tabuListMutex);
            localTabuList.push_back(currentSolution.permutation);
...
// Zrównoleglona funkcja Tabu Search.
void parallelTabuSearch(const vector<vector<int>>& processingTimes, int maxIterations, int tabuSize, int numThreads) {
    ...
    for (int t = 0; t < numThreads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            // Aktualizacja najlepszych rozwiązań dla danego wątku.
...
int main() {
    ...
    // Uruchomienie zrównoleglonego algorytmu Tabu Search.
